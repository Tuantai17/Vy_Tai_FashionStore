...
export default function AdminCoupons() {
  ...
  const loadCoupons = async (targetPage = 1) => {
    try {
      setLoading(true);
      setError("");
      const res = await apiGet("/admin/coupons", { auth: true, query: { page: targetPage } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await apiPut(`/admin/coupons/${form.id}`, payload, { auth: true });
        setMessage("Đã cập nhật mã giảm giá.");
      } else {
        await apiPost("/admin/coupons", payload, { auth: true });
  ...
  const handleDelete = async (coupon) => {
    ...
    try {
      await apiDelete(`/admin/coupons/${coupon.id}`, { auth: true });
